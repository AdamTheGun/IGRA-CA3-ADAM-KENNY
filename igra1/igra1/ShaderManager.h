...
public:	// general accessors
	...
	ID3D11VertexShader* VSTerrain(){return mpVSTerrain;}
	ID3D11InputLayout*	LayoutTerrain(){return mpLayoutTerrain;}
	ID3D11PixelShader*	PSTerrain(){return mpPSTerrain;}
	ID3D11PixelShader*	PSTerrainBasic(){return mpPSTerrainBasic;}

	// skybox items
	ID3D11VertexShader* VSSkybox(){return mpVSSkybox;}
	ID3D11InputLayout*	LayoutSkybox(){return mpLayoutSkybox;}
	ID3D11PixelShader*	PSSkybox(){return mpPSSkybox;}
	...
	struct cbMaterial
{
	...
	BOOL gUseTexture;
		// needs to be a 4 bytes, so using BOOL rather than bool
	BOOL gEnableReflection;
		// needs to be a 4 bytes, so using BOOL rather than bool
	BOOL pad1,pad2;	// need 8 more bytes (2 BOOL's) as padding
}; 
...
private:	// internal data
	...
	// skybox
	CComPtr<ID3D11VertexShader> mpVSSkybox;
	CComPtr<ID3D11InputLayout> mpLayoutSkybox;
	CComPtr<ID3D11PixelShader> mpPSSkybox;

	CComPtr<ID3D11VertexShader> mpVSTerrain;
	CComPtr<ID3D11InputLayout> mpLayoutTerrain;
	CComPtr<ID3D11PixelShader> mpPSTerrain;
	CComPtr<ID3D11PixelShader> mpPSTerrainBasic;
...
class TerrainMaterial: public ShaderMaterial
{
public:	// following variables MUST be set before use
	CComPtr<ID3D11ShaderResourceView> mpTexBase;
                          	///< the base/blend texture
	CComPtr<ID3D11ShaderResourceView> mpTexLayerArray;
                        	///< the texture layer array
	CComPtr<ID3D11ShaderResourceView> mpTexLight;
	bool mUseTextureBlend;	// whether to use texture blends

public:	// common functions
	// applies all shaders, constant buffers, textures & samplers
	void Apply(ID3D11DeviceContext* pContext);
};
